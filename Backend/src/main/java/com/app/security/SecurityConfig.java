...
public class SecurityConfig {
	...
	public SecurityFilterChain authorizeRequests(HttpSecurity httpSecurity) throws Exception {
	    ...
	    httpSecurity.cors().and().csrf().disable().exceptionHandling()
	            ...
	            .antMatchers("/customers/**").permitAll() // Accessible to all
	            .antMatchers("/vendors/**", "/tiffins/**").permitAll() // Accessible to all
	            .antMatchers("/contact-us/**", "/add-donation-meal/**").permitAll() // 🚀 ALLOW CONTACT-US ENDPOINT
	            .antMatchers("/home", "/orders/**", "/auth/**", "/swagger*/**", "/v*/api-docs/**", "/subscription/**", "/customerplan/**", "/donations/**").permitAll()
